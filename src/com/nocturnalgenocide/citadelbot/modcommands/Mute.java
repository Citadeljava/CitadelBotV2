    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        if(args[0].equalsIgnoreCase(Bot.prefix + "mute")) {
            ...
            if(!member.hasPermission(Permission.MANAGE_ROLES)) {
                ...
                if(args.length <= 1) {
                    channel.sendMessage("Usage: c:mute <@member>").queue();
                } else {
                    Member target = event.getMessage().getMentionedMembers().get(0);
                    Role muted = event.getGuild().getRolesByName("Muted", true).get(0);
                    event.getGuild().addRoleToMember(target, muted).queue();

                    channel.sendMessage("Successfully Muted " + target.getAsMention()).complete().delete().queueAfter(10, TimeUnit.SECONDS);
                }
            }
        }
    }
